...
public class ApplicationMenu{
	...
	private void uiModifyElements() {
		System.out.println("Ce id are plansa de lucru unde doriti sa modificati culoarea elementelor");
		plansa.afisareId();
		int id = consoleScanner.nextInt();
		System.out.println("Ce culoare doriti sa modificati?");
		String el = consoleScanner.next();
		PlansaDeLucru plansaTemporara = plansa.cautareId(id);
		for(IGraficElement elGrf:plansaTemporara.elemente)
		{
			if(elGrf instanceof Shape) {
				((Shape) elGrf).setCuloare(el);
			}
		}
	...
	private void uiAddElement() {
		...
		plansa.afisareId();
		int id = consoleScanner.nextInt();
		System.out.println("Ce element doriti sa introducti?\n1.patrat\n2.dreptungi\n3.triunghi\n4.cerc\n5.plansa de lucru");
		int el =consoleScanner.nextInt() ;
		switch(el)
		{
		case 1:
			System.out.println("S-a creat un nou patrat");
			plansa.cautareId(id).addElement(new Patrat("negru"));
			break;
		case 2:
			System.out.println("S-a creat un nou dreptunghi");
			plansa.cautareId(id).addElement(new Dreptunghi("negru"));
			break;
		case 3:
			System.out.println("S-a creat un nou triunghi");
			plansa.cautareId(id).addElement(new Triunghi("negru"));
			break;
		case 4:
			System.out.println("S-a creat un nou cerc");
			plansa.cautareId(id).addElement(new Cerc("negru"));
			break;
		case 5:
